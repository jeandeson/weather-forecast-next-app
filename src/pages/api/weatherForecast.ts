...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ForecastData>) {
    ...
    data.daily.forEach((item: IForecasApiResponse) => {
        ...
        forecastValues.forecastData.push(
            new Weather(date, Math.round(item.temp.day), item.weather[0].description, item.weather[0].main)
        );
